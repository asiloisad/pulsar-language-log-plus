...
class LogView {
  ...
  buildElement() {
    ...
    this.element = createElement('div', { className: 'language-log-plus-view', tabIndex: -1 }, [
      ...
      createElement('header', { className: 'header' }, [
        ...
        this.closeButton,
        createElement('span', { className: 'header-item options-label pull-right', text: 'Level Filters' }),
      ...
      createElement('section', { className: 'input-block filter-container' }, [
        ...
        createElement('div', { className: 'input-block-item' }, [
          createElement('div', { className: 'btn-group btn-group-filter' }, [this.filterButton]),
          createElement('div', { className: 'btn-group btn-group-tail' }, [this.tailButton]),
          createElement('div', { className: 'btn-group btn-group-case' }, [this.caseSensistiveButton]),
  ...
  handleConfigChanges() {
    this.disposables.add(atom.config.onDidChange('language-log-plus.adjacentLines', () => {
      this.confirm();
    }));
  }
  ...
  toggleTail() {
    atom.config.set('language-log-plus.tail', !atom.config.get('language-log-plus.tail'));
    this.updateButtons();
    return this.tail();
  }

  toggleCaseSensitivity() {
    atom.config.set('language-log-plus.caseInsensitive', !atom.config.get('language-log-plus.caseInsensitive'));
    this.updateButtons();
    return this.confirm();
  }
  ...
  updateButtons() {
    this.tailButton.classList.toggle('selected', atom.config.get('language-log-plus.tail'));
    this.caseSensistiveButton.classList.toggle('selected', !atom.config.get('language-log-plus.caseInsensitive'));
  ...
  tail() {
    if (!(atom.config.get('language-log-plus.tail') && this.textEditor)) {
      return;
    }
